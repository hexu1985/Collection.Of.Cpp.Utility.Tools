...
DDSDataSharing.*
DDSDiscovery.*
EndpointPartitioning.*
DDSFindTopicTest.*
DDSStatus.*
DDSStatistics.*
StatisticsDomainParticipant.*
AcknackQos.*
Discovery.*
KeyedTopic.*
LatencyBudgetQos.*
...
BlackBox.*
TransportTCP.*
DDSContentFilter/DDSContentFilter.*
DDSDataReader/DDSDataReader.*
DDSDataWriter/DDSDataWriter.*
DDSStatus/DDSStatus.*
OwnershipQos/OwnershipQos.*
PersistenceLargeData/PersistenceLargeData.*
PersistenceGuid/PersistenceGuid.*
PropertyQos/PropertyQos.*
UserDataQos/UserDataQos.*
...
do
    ./BlackboxTests_DDS_PIM --gtest_filter="${case_set}:-${exclude_filter_str}"
    test_result=$?
    if [ ${test_result} -ne 0 ]
    then
