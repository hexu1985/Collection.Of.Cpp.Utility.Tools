...
| V4.1 | 2026-09-16 | AI Assistant | 基于 Daytime 示例完善 9.3 典型使用示例，展示空参数调用、方法绑定、生命周期管理等真实用法 |

> 说明：本版本仅更新 **第 9.3 节 典型使用示例**，其余章节内容与 V4.0 保持一致，为保持文档完整性，此处完整呈现全文档。
...
└── examples/
    ├── daytime/
    │   ├── DaytimeClient.h / DaytimeClient.cpp
    │   ├── DaytimeServer.h / DaytimeServer.cpp
...
### 9.3 典型使用示例（Daytime 完整示例）

本节以 `Daytime` 服务为例，展示框架的完整使用方式。示例覆盖了**空参数方法（`VoidType`）**、**方法绑定（`std::bind`）**、**服务可用性轮询**、**错误码检查**等真实业务模式。

#### 9.3.1 示例概述

- **服务名**：`daytime.service`
- **方法名**：`daytime`
- **请求参数**：`soa_on_dds::VoidType`（空类型，无需序列化）
- **响应结果**：`std::string`（框架内置支持，无需自定义适配函数）
...
void DaytimeServer::daytime_handle(const VoidType& arg, std::string& res)
{
    ...
    std::cout << "DaytimeServer::daytime_handle end" << std::endl;
}
...
public:
    ...
    // 初始化：创建 RPC Client 并建立 DDS 连接
    bool init();

    // 业务接口：调用远程 daytime 方法
    std::string get_daytime();

    // 查询 RPC 客户端是否已就绪（DDS 发现完成）
    bool isReady();
...
{
    // 创建 RPC Client：
    //   client_name  = "daytime_client"（用于生成 client_id）
    //   service_name = "daytime.service"（用于构建 Topic）
...
| 要点 | 说明 |
| :--- | :--- |
| **空参数类型 `VoidType`** | 框架内置 `VoidType` 及其序列化适配函数（`SerializeToVector` 清空输出，`DeserializeFromVector` 直接返回 true），无参方法无需自定义任何类型。 |
| **`std::string` 作为结果** | 框架内置对 `std::string` 的序列化支持（字符逐一拷贝为 `uint8_t`），无需编写适配函数。 |
| **`std::bind` 绑定成员函数** | `register_method` 接受 `std::function<void(const Argument&, Result&)>`，`std::bind(&Class::method, this, _1, _2)` 可方便地绑定成员函数。也可直接使用 lambda。 |
| **服务端线程池大小** | `create_rpc_server("daytime.service", 3)` 指定 3 个 worker 线程。框架内部限制为 `[1, 5]`，实际传入更大值会被裁剪。 |
| **服务可用性轮询** | `is_ready()` 返回 false 时调用 `call` 会立即返回 `SERVICE_NOT_AVAILABLE`。示例中使用 100ms 轮询等待 DDS 发现完成，避免首次调用失败。 |
| **错误码检查** | `get_daytime()` 中检查 `error_code != SUCCESS`，并通过 `EprosimaRpcUtility::error_code_to_string` 转换为可读字符串，便于日志与排错。 |
| **`localtime_r` 可重入** | 服务端 `daytime_handle` 可能被多个 worker 线程并发调用，使用 POSIX 可重入版本 `localtime_r` 而非 `localtime`，避免数据竞争。 |
| **生命周期管理** | `DaytimeClient` / `DaytimeServer` 均在析构时 `reset()` 智能指针，触发 `EprosimaRpcClient` / `EprosimaRpcServer` 的析构，自动清理 DDS 实体与内部线程。 |
| **`serve()` 阻塞退出** | 服务端通过 `std::cin >> aux` 阻塞，直到用户输入任意整数后 `serve()` 返回，`main` 结束，触发 `~DaytimeServer()`。 |

#### 9.3.5 扩展建议

若业务需要传递结构化参数，只需定义自定义类型并提供序列化适配函数即可。例如：
...
bool SerializeToVector(const LoginRequest& req, std::vector<uint8_t>& out) { /* ... */ }
bool DeserializeFromVector(LoginRequest& req, const std::vector<uint8_t>& in) { /* ... */ }
bool SerializeToVector(const LoginResponse& res, std::vector<uint8_t>& out) { /* ... */ }
bool DeserializeFromVector(LoginResponse& res, const std::vector<uint8_t>& in) { /* ... */ }
```

之后即可在 `register_method<LoginRequest, LoginResponse>` 与 `call<LoginRequest, LoginResponse>` 中直接使用，无需修改框架代码。
