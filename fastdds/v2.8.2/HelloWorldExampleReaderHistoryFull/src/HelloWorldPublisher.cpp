...
public:
    ...
    bool init()
    {
        ...
        // 配置DataWriter QoS - 设置大History以快速填满订阅者缓存
        DataWriterQos writer_qos = DATAWRITER_QOS_DEFAULT;
        writer_qos.history().kind = KEEP_LAST_HISTORY_QOS;
        writer_qos.history().depth = 100;  // 大History深度
        writer_qos.reliability().kind = RELIABLE_RELIABILITY_QOS;
        writer_qos.publish_mode().kind = ASYNCHRONOUS_PUBLISH_MODE;  // 异步发布提高速度
            ...
            uint32_t samples)
    {
        ...
        while (samples_sent < samples)
        {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(10));  // 快速发送
        }
    }
};
