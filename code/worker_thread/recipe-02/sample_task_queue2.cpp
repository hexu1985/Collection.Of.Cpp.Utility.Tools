...
int main()
{
    ...
    for (int i = 0; i < 10; i++) {
        task_queue.PushTask(Task(print_int, i));
        task_queue.PushTask(Task(print_string, str));
        task_queue.PushTask(Task(&Foo::print, &foo));
        task_queue.PushTask(Task(&Foo::print, pfoo));
    }
    task_queue.PushTask(Task(process_exit));
    proc_thread.join();
    return 0;
}
