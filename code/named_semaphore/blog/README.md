...
本文主要根据《UNIX网络编程卷2  进程间通信  (第2版)》这本书中的原理，
介绍如何基于C++来封装Posix信号量的类和接口。

实现原理主要参考《UNIX网络编程卷2  进程间通信  (第2版)》这本书中的
10.15章节，使用内存映射I/O实现信号量。不过涉及如下细节调整：

- 主要介绍有名信号量的实现，支持进程间同步
- 接口参考boost::named_semaphore，但是略有调整
- 底层基于Posix共享内存实现：依赖《C++封装Posix API之共享内存》文章中SharedMemory模板类的实现
